...
>(({ className, externalState, ...props }, ref) => {
  ...
  const handleClick = () => {
    const nextState = state === "unchecked" ? "checked" : 
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        {state === "checked" && <Check className="h-4 w-4 text-supreme-blue-700" />}
        {state === "indeterminate" && <Minus className="h-4 w-4 text-supreme-blue-700" />}
      </div>
    </CheckboxPrimitive.Root>
  )
})
